...
/** @ignore */
function pisgah_head() {
}
add_action('wp_head', 'pisgah_head');
...
<?php
	register_sidebar( array(
	    'id'          => 'store-categories',
	    'name'        => __( 'Store Sidebar', $text_domain ),
	    'description' => __( 'This sidebar is located in the Store.', $text_domain ),
	) );

function pisghaShopSidebar () {
	echo "<div id='sidebar' class='add_shadow'>
				<div id='sidebar_title'><a href='/shop/'>GOODS</a></div>
				<div id='sidebar_content'>
				<ul class='sidebar_nav'>";
	if ( !dynamic_sidebar('store-categories')) {
		echo "<li>{static sidebar item 1}</li>";		
	}	
	
  $pages_array = wp_list_pages(array(
  	'title_li'=>'',
    'echo' => '0',
    'include' => '2223',
   )
	...
	add_action('jigoshop_before_main_content', create_function('', 'echo "<div id=\"main_stage\">
	<div id=\"internal_bg\"><div id=\"internal_shop\" class=\"add_shadow\">";'), 10);
	add_action('jigoshop_after_main_content', create_function('', 'echo "</section>";'), 10);
	add_action('jigoshop_sidebar', 'pisghaShopSidebar', 10);
