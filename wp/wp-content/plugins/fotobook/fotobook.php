...
function fb_display($content) {
	...
	if($post_id == $albums_page_id) {
			return fb_display_album($content, "1245");
...
function fb_display_main($content) {
	...
	if($page_count == 1) {
		...
		if($prev_link !== false)
			//$album_link.(strstr($album_link, '?') ? '&amp;album_p='.($prev_link) : 
			$prev_link = '?album_p='.($prev_link);
		$next_link = $curr_page + 1 <= $page_count ? $curr_page + 1 : false;
		if($next_link)
			// Removed to make single album nav work correctly
			// $album_link.(strstr($album_link, '?') ? '&amp;album_p='.($next_link) : 
			$next_link = '?album_p='.($next_link);
		$pagination = '';
		for($i = 1; $i <= $page_count; $i++) {
			if($i == $curr_page)
				$pagination .= '<b>'.$i.'</b>';
			else {
				//$album_link.(strstr($album_link, '?') ? '&amp;album_p='.$i : 
				$link = '?album_p='.$i;
	...
	<div id="fotobook-credits" style="text-align: right;">
		<small style='visibility: hidden;'>Powered by <a href="http://www.aaronharp.com/dev/wp-fotobook/">Fotobook</a></small>
	</div>
<?php
...
function fb_display_album($content, $page_id) {
	...
	if($page_count == 1) {
		...
		$prev_link = $curr_page > 1 ? $curr_page - 1 : false;
		if($prev_link !== false)
			//$page_link.(strstr($page_link, '?') ? '&amp;album_p='.($prev_link) : 
			$prev_link = '?album_p='.($prev_link);
		$next_link = $curr_page < $page_count ? $curr_page + 1 : null;
		if($next_link)
			//$page_link.(strstr($page_link, '?') ? '&amp;album_p='.($next_link) : 
			$next_link = '?album_p='.($next_link);
		$pagination = '';
		for($i = 1; $i <= $page_count; $i++) {
			if($i == $curr_page)
				...
				//$page_link.(strstr($page_link, '?') ? '&amp;album_p='.$i : 
				$link = '?album_p='.$i;
				$pagination .= "<a href='$link'>".($i)."</a>";
			}
		}
